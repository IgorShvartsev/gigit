...
class Layout {
  ...
  { 
    ...
    // !!! HACK to make possible to pass param $data of  custom controller
    if ( isset($GLOBALS['CI']->data) && is_array($GLOBALS['CI']->data) )
    {
        foreach($GLOBALS['CI']->data as $data) {
            $GLOBALS['CI']->load->_ci_cached_vars = array_merge($GLOBALS['CI']->data, $GLOBALS['CI']->load->_ci_cached_vars);
        }
    }
    ...
    if ($CI->layout->active AND $CI->layout->layout AND file_exists(APPPATH . $CI->layout->layout_dir . $CI->layout->layout . EXT)) {
      ...
      // return layout
      $buffer = $CI->load->view('../' . $CI->layout->layout_dir . $CI->layout->layout, array($CI->layout->layout_var => $buffer), TRUE);
    }
